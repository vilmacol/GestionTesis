(function (global, factory) {
  ...
  const zeroRoadmapChartInit = () => {
    ...
    if (zeroRoadMapEl) {
      ...
      const scaleRadios = zeroRoadMapEl.querySelectorAll(
        "input[name=scaleView]"
      );

      const progressCheck = zeroRoadMapEl.querySelector(
        "[data-gantt-progress]"
      );
      ...
      if (lists.length) {
        lists.forEach((el) => {
          ...
          const updateListControls = () => {
            listInfo &&
              (listInfo.innerHTML = `${list.i} de ${numberOfcurrentItems} <span class='text-600'>total de items </span>${totalItem}`);
